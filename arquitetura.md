...
- **[Linhas 14-60 ~]** `// 🚀 [INÍCIO: PWA - CONSTANTES DE CACHE E ASSETS ESSENCIAIS]` -> `const CACHE_NAME = 'notas-pwa-v71';` · `const TIMEOUT_MS = 3000;` · `const OFFLINE_HTML` · `const ESSENCIAIS = [...]` (inclui `./fontes.js`, `./conta.js`, `./sync/chaves.js`, `./sync/sync-cliente.js` e `./mapa/mapa-cores.js`)
...
- **[Linhas 1-78 ~]** `# 🚨 [INÍCIO: BACKEND - SEGURANÇA (CABEÇALHOS)]` -> `_DIRETIVAS_CSP` (`default-src 'self'`, `connect-src 'self' ws: wss:`, `frame-src` só o YouTube nocookie, `object-src 'none'`, `frame-ancestors 'none'`; `style-src 'unsafe-inline'` porque o app usa `style` inline) · `_hashes_inline` (`sha256-` de cada `<script>` **inline** — a CSP é cacheável: sem nonce) · `csp_do_html` (cacheado por arquivo) · `cabecalhos_de_seguranca` (CSP no HTML + `nosniff`/`Referrer-Policy`/`X-Frame-Options`/`COOP`/`Permissions-Policy` sempre; **HSTS só com `SESSAO_SEGURA=true`**)
- **[Linhas 80-132 ~]** `# 🔄 [INÍCIO: BACKEND - APP/ROTAS]` -> `app = FastAPI(...)` · handler de `RequestValidationError` (mantém `detail` como **string**) · `GET /health` (`{ok, driver, modo, supabase, avisos}`) · `app.include_router(auth_router)` + `sync_router` + `ws_router` + `assets_router` (**antes** do estático — a ordem importa)
- **[Linhas 132-206 ~]** `(continuação) -> estático`: `TIPOS` (media types) · `ARQUIVOS_RAIZ` + `PASTAS_PWA` (`notes`/`mapa`/`sync`) = **allowlist** · `_normalizar`/`_permitido` (bloqueia `..` e tudo fora do PWA) · `servir()` (404 em JSON; **`cabecalhos_de_seguranca` em TODA resposta**; `sw.js` com `Cache-Control: no-store` + `Service-Worker-Allowed`; `.html` com `no-cache`) · `GET /` e `GET /{caminho:path}`
...
**Propósito:** Ambiente e operação. `requirements.txt` = FastAPI/Uvicorn/Pydantic/dotenv/httpx/**python-multipart** (obrigatório para o upload dos anexos) + `supabase` e `psycopg[binary]` **opcionais** + pytest (resolvido no Python 3.14.5: FastAPI 0.141.1, pydantic 2.13.5 + pydantic-core 2.46.5, httpx 0.28.1, pytest 9.1.1, supabase 2.31.0, psycopg 3.3.6). `.env.example` = modelo **sem valores** (com `SUPABASE_DB_URL`, `ANEXO_LIMITE_MB=25` e `ANEXO_COTA_MB=900`). `README.md` = como subir, como ligar o Supabase (`npm run verificar:backend`) e como conferir que nenhuma chave vazou.
- **[requirements.txt 1-38 ~]** `(sem comentário de ancoragem)` -> runtime + opcionais + dev (nota **R1**: se um wheel faltar, o driver cai no caminho httpx) · **[requirements.lock.txt ~]** `pip freeze` do ambiente validado (49 pacotes) para reprodução exata
- **[.env.example 1-44 ~]** `(sem comentário de ancoragem)` -> `SUPABASE_URL`/`ANON_KEY`/`SERVICE_ROLE_KEY`/`BUCKET_ANEXOS`/`DB_URL`, `SESSAO_*`, `ORIGENS_PERMITIDAS`, `HOST`/`PORTA`, `DRIVER`, `RATE_*`, `ANEXO_LIMITE_MB`/`ANEXO_COTA_MB`
...
- **[Arquivo inteiro ~]** `/* INVARIANTE do projeto: SEM login e com o BACKEND DESLIGADO` -> `tests/modo_local_sem_backend.cjs` (boot REAL com `/api/**` ABORTADO: sobe e edita, salva no LocalStorage, **não** cria fila, **nenhum** socket aberto e o reload preserva o texto — prova o §0.5/R7)
