...
- **[Linhas 1-83 ~]** `# 🚨 [INÍCIO: BACKEND - SEGURANÇA (CABEÇALHOS)]` -> `_DIRETIVAS_CSP` (`default-src 'self'`, `connect-src 'self' ws: wss:`, `frame-src` = YouTube nocookie **+ `https://accounts.google.com`** (o botão do GIS é um iframe dele), `object-src 'none'`, `frame-ancestors 'none'`; `style-src 'unsafe-inline'` porque o app usa `style` inline) · `_hashes_inline` (`sha256-` de cada `<script>` **inline** — a CSP é cacheável: sem nonce) · `csp_do_html` (cacheado por arquivo; **`script-src` libera `https://accounts.google.com`** — só a origem exata do GIS) · `cabecalhos_de_seguranca` (CSP no HTML + `nosniff`/`Referrer-Policy`/`X-Frame-Options`/`COOP`/`Permissions-Policy` sempre; **HSTS só com `SESSAO_SEGURA=true`**)
