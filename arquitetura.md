...
**Partes:** `1) Boot/Shell` ✅ · `2) Motor Notas` ✅ · `3) Tabelas` ✅ · `4) Mapa Mental` ✅ · `5) Testes/Ferramentas` ✅ · `6) Backend Python (login/sync)` ✅ bloco I · `7) Conta do usuário (login opcional)` ✅ bloco II · `8) Sincronização (fila + snapshot/push)` ✅ bloco II
...
- **[Linhas 14-60 ~]** `// 🚀 [INÍCIO: PWA - CONSTANTES DE CACHE E ASSETS ESSENCIAIS]` -> `const CACHE_NAME = 'notas-pwa-v68';` · `const TIMEOUT_MS = 3000;` · `const OFFLINE_HTML` · `const ESSENCIAIS = [...]` (inclui `./fontes.js`, `./conta.js`, `./sync/chaves.js`, `./sync/sync-cliente.js` e `./mapa/mapa-cores.js`)
...
**Propósito:** Conta do usuário — **login OPCIONAL** por e-mail/senha. É o rosto do backend Python: o **botão de conta** na moldura fixa do topo (substituiu a seta ‹) e o **diálogo de conta** (entrar / criar conta / sair). Deslogado, o app é **exatamente o de hoje** (LocalStorage, offline-first); o backend nunca é obrigatório.
- **[Linhas 1-26 ~]** `"""`-equivalente em JS (`/** … */`) + `// 🚨 [INÍCIO: CONTA - ESTADO E CHAMADAS À API]` -> `window.notasConta = { logado, email, csrf, sync: { estado, pendentes } }` (**espelho — nunca guarda token**: a sessão é cookie `HttpOnly`) · `ICONE_PESSOA` (SVG)
...
- **[Linhas 71-105 ~]** `// 🚨 [INÍCIO: CONTA - SESSÃO ...]` -> `p.contaAplicar(dados)` (espelha, atualiza a UI e **dispara o sync**: `syncAoEntrar()` ao entrar, `syncAoSair()` ao sair — gancho da seção 5) · `p.contaVerificar()` (**SILENCIOSO no boot — P62**: sem backend, sobe deslogado e utilizável) · `p.contaEntrar` · `p.contaRegistrar` · `p.contaSair` (idempotente)
- **[Linhas 106-118 ~]** `// ⚡ [INÍCIO: CONTA - BOTÃO DO CABEÇALHO]` -> `p.contaAtualizarBotao()` (ícone + `.conta-btn-nome` = "Entrar" ou o e-mail; `data-logado`; `title`/`aria-label` completos)
- **[Linhas 120-277 ~]** `// ⚡ [INÍCIO: CONTA - DIÁLOGO (entrar / criar conta / sair)]` -> helpers (`criar`/`paragrafo`/`botao`/`campo`) · `p.montarContaLogado` (e-mail + situação do sync + "Sair") · `p.montarContaDeslogado` (formulário com Entrar/Criar conta, erro inline `role="alert"`, botões desabilitados durante o envio) · `p.contaMontarDialogo` (redesenha só os `[data-conta]`) · `p.contaAbrirDialog` (reusa **`notesExtraDialog`**, como o "Modelos") · `p.contaAtualizarDialogo` · `p.contaTextoDoSync` (`Sincronizado` / `Enviando N alterações…` / `Offline — N na fila` / `Sessão expirada` / `Só neste aparelho` — sem hora)
- **[Linhas 279-305 ~]** `// 🚀 [INÍCIO: CONTA - INSTALAÇÃO (installConta)]` -> `p.instalarContaUI()` (liga o clique do botão **uma vez**, atualiza o rótulo, recheca a sessão no evento `online` e chama `p.contaVerificar()` no boot) · `installConta(App)` chamado por `installNotesFeatures()`
...
- **[Linhas 177-443 ~]** `// 🔄 [INÍCIO: SYNC - RECEBIMENTO ...]` -> `syncSnapshot(desdeRev)` · `syncBaixarEntidade`/`syncBaixarNota` (nota `somenteNuvem`) · `notaLocal()` (nuvem→local) · **`syncAplicarEntidades()`** (marca `syncAplicando` para a aplicação NÃO voltar para a fila; re-renderiza chips/área do mapa) · `syncAplicarNotas` (marca `syncEstale` — o editor aberto ficou desatualizado) /`syncAplicarConfiguracoes` (guarda o **texto CRU** do ajuste) /`syncAplicarPastas`/`syncAplicarModelos`/`syncAplicarMapas` (índice + `notas-pwa-mapa-<id>`) · `syncAplicarItem(conflito)` · **`syncRefazerEditor()`** (só com a nota ABERTA: neutraliza a sessão e **reabre pelo caminho do app** — sem escrever `innerHTML` cru) · **`gravarNotasLocaisComCota()`** (tenta tudo; o que não couber vira `somenteNuvem`)
- **[Linhas 439-575 ~]** `// 🔄 [INÍCIO: SYNC - COLETA E 1º LOGIN ...]` -> `syncDadosDaNota`/`syncEnfileirarNota` · **`syncColetarOps()`** (varre pastas, índice de mapas + grafos, modelos (2 tipos) e ajustes — o mapa de chaves decide o que entra) · **`syncSubirTudo()`** (lotes de 60, progresso no diálogo) · **`syncAoEntrar()`** (snapshot → **1º login sobe tudo** com a marca `notas-pwa-sync-conta` → drena → liga `syncPronto`) · `syncAoSair()` (a fila é da conta; desliga `syncPronto`)
- **[Linhas 577-644 ~]** `// 🔄 [INÍCIO: SYNC - GANCHOS NO APP ...]` -> `p.apiCall` (a MESMA gravação também entra na fila — **exceto** com `syncAplicando`/`syncEstale`) · `p.openNotesModal` (baixa o texto da nota `somenteNuvem` antes de abrir; sem conexão avisa e abre; no fim o app já renderizou → `syncEstale = false`) · `p.notasBackend` (driver local + `remoto`) · estado inicial `local` (**`installSync` roda como função simples: aqui `this` NÃO é a instância — usar `p` com `call`**)

---

# Nome do Arquivo: backend/ (área) — API Python OPCIONAL (login + sync Supabase)
**Propósito:** Backend **opcional** em Python (FastAPI). O PWA continua **local-first**: sem login e com o backend desligado o comportamento é **idêntico** ao de hoje; só o Python fala com o Supabase (a chave `service_role` **nunca** chega ao front). **Bloco I = seções 1–3** (fundação + Supabase real + login); as seções 4–10 (conta/sync/WebSocket/anexos) vêm depois.

## Implementação: Configuração e escolha do driver
- **[Linhas 1-120 ~]** `"""🗺️ COMPONENTE: Configuracao…"""` + `# 🔄 [INÍCIO: BACKEND - CONFIG]` -> `RAIZ` · `@dataclass Configuracao` (driver, porta/host, `supabase_*`, `sessao_*`/`csrf_header`/`seguro`, `origens_permitidas`, `rate_*`) · propriedades `tem_supabase` / `driver` (`auto` → `supabase` se houver credencial, senão `memory`) / `modo` / `avisos` · `montar()` · `obter_config()` (singleton) · `recarregar()`. Carrega `backend/.env` e (se existir) o `.env` da raiz com `override=False` — **sem** segredo no código.
...
# Nome do Arquivo: backend/app/seguranca.py
...
- **[Linhas 1-99 ~]** `# 🚨 [INÍCIO: BACKEND - IDENTIDADE]` -> `ErroIdentidade(mensagem, status)` · `_hash_senha`/`_conferir_senha` (PBKDF2-SHA256, 120k) · `class IdentidadeMemoria` (`registrar` devolve id novo; `autenticar` compara hash; `usuario`; `limpar`) — bloco dividido: fecha em **`[FIM: BACKEND - IDENTIDADE - PARTE 1]`**
- **[Linhas 102-209 ~]** `# 🚨 [INÍCIO: BACKEND - IDENTIDADE]` -> `class IdentidadeGoTrue` (`base_url = {url}/auth/v1`; **cadastro pelo endpoint ADMIN** `/admin/users` com `email_confirm: true`; login em `/token?grant_type=password`; `usuario()` em `/admin/users/{id}`; `remover()` só para limpeza do `verificar.py`; erros de rede → `ErroIdentidade(503)`; mensagens **genéricas** para não revelar se o e-mail existe) · `obter_identidade()` (singleton) · `definir_identidade()` / `reiniciar_identidade()`

# Nome do Arquivo: backend/app/auth.py
**Propósito:** Rotas `/api/auth`: `registrar` · `login` · `logout` · `me`. Erros **sempre** `{"detail": "mensagem"}`; cookie `HttpOnly`; CSRF nas escritas; rate-limit em login/registro.
- **[Linhas 1-91 ~]** `# 🚨 [INÍCIO: BACKEND - AUTH]` -> `router = APIRouter(prefix="/api/auth")` · `Credenciais` (Pydantic com defaults, para **nunca** devolver 422 em lista) · `_validar()` (e-mail/senha; 400) · `_limitar()` (429) · `_abrir_sessao()` · `POST /registrar` (204 + cookie; mensagem genérica "Nao foi possivel criar a conta") · `POST /login` (**401 com a MESMA mensagem** para senha errada e e-mail inexistente; 503 se a identidade cair) · `POST /logout` (**idempotente**; exige CSRF quando há sessão) · `GET /me` (401 sem sessão; devolve `{id, email, criado_em, csrf}`)

# Nome do Arquivo: backend/app/main.py
**Propósito:** App FastAPI: `/health`, as rotas `/api/auth` e o **estático do PWA por allowlist** (o `backend/`, `docs/`, `tests/`, `node_modules/`, `.git/` e qualquer `.env` respondem **404**).
- **[Linhas 1-53 ~]** `# 🔄 [INÍCIO: BACKEND - APP/ROTAS]` -> `app = FastAPI(...)` · handler de `RequestValidationError` (mantém `detail` como **string**) · `GET /health` (`{ok, driver, modo, supabase, avisos}`) · `app.include_router(auth_router)` + `app.include_router(sync_router)` (**antes** do estático — a ordem importa)
- **[Linhas 53-115 ~]** `(continuação) -> estático`: `TIPOS` (media types) · `ARQUIVOS_RAIZ` + `PASTAS_PWA` (`notes`/`mapa`/`sync`) = **allowlist** · `_normalizar`/`_permitido` (bloqueia `..` e tudo fora do PWA) · `servir()` (404 em JSON; `sw.js` com `Cache-Control: no-store` + `Service-Worker-Allowed`; `.html` com `no-cache`) · `GET /` e `GET /{caminho:path}`
- **[Linhas 118-132 ~]** `# 🚀 [INÍCIO: BACKEND - BOOT (uvicorn)]` -> `main()` (imprime os avisos e sobe em `HOST:PORTA`) · `if __name__ == "__main__"`
...
- **[migrar.py 1-81 ~]** `# 💾 [INÍCIO: BACKEND - MIGRAR (SCHEMA)]` -> `SCHEMA` (caminho do `schema.sql`) · `TABELAS` (as 9) · `sql_do_schema()` · `aplicar_com_psycopg(dsn)` (quando há `SUPABASE_DB_URL`) · `conferir_via_rest(config)` (GET limitado por tabela, sem depender de psycopg) · `instruir_manual()` (passo a passo do SQL Editor) · `main()` (1 = faltou tabela/credencial)
...
**Propósito:** Testes do backend **sem rede, sem credencial e sem banco**: o `conftest.py` força `DRIVER=memory` e apaga as variáveis do Supabase; o Supabase é coberto por um **PostgREST emulado** (`httpx.MockTransport`).
- **[conftest.py 1-52 ~]** `# 🧪 [INÍCIO: TESTE - BACKEND/CONFTEST]` -> `DRIVER=memory` **antes** de importar o backend; `RAIZ`/`AQUI` no `sys.path`; fixtures `config_memoria`, `repo` e **`repo_qualquer`** (roda o MESMO contrato nos dois drivers)
...
- **[test_health.py 1-62 ~]** `# 🧪 [INÍCIO: TESTE - BACKEND/TEST_HEALTH]` -> `/health` em `memory`; `/` serve o `index.html`; módulos do PWA com MIME certo; `sw.js` com `no-store` + `Service-Worker-Allowed`; **11 caminhos proibidos = 404** (`.env`, `backend/`, `docs/`, `tests/`, `node_modules/`, travessia de caminho)
...
**Propósito:** Ambiente e operação. `requirements.txt` = FastAPI/Uvicorn/Pydantic/dotenv/httpx + `supabase` e `psycopg[binary]` **opcionais** + pytest (resolvido no Python 3.14.5: FastAPI 0.141.1, pydantic 2.13.5 + pydantic-core 2.46.5, httpx 0.28.1, pytest 9.1.1, supabase 2.31.0, psycopg 3.3.6). `.env.example` = modelo **sem valores** (com `SUPABASE_DB_URL` opcional). `README.md` = como subir, como ligar o Supabase (`npm run verificar:backend`) e como conferir que nenhuma chave vazou.
- **[requirements.txt 1-36 ~]** `(sem comentário de ancoragem)` -> runtime + opcionais + dev (nota **R1**: se um wheel faltar, o driver cai no caminho httpx) · **[requirements.lock.txt ~]** `pip freeze` do ambiente validado (49 pacotes) para reprodução exata
- **[.env.example 1-39 ~]** `(sem comentário de ancoragem)` -> `SUPABASE_URL`/`ANON_KEY`/`SERVICE_ROLE_KEY`/`BUCKET_ANEXOS`/`DB_URL`, `SESSAO_*`, `ORIGENS_PERMITIDAS`, `HOST`/`PORTA`, `DRIVER`, `RATE_*`
...
- **[Arquivo inteiro ~]** `/* Sincronização de NOTAS por HTTP (seção 5), contra o backend REAL:` -> `tests/sync_snapshot.cjs` (sobe `uvicorn` com `DRIVER=memory`: offline não cria fila · 1º login sobe tudo · edição → fila → `ack` → nuvem · 2º aparelho vê · sair limpa a fila) — **falha conhecida (P98)**: no aparelho NOVO o último passo acusa a nota vazia (autosave do boot vence a versão da nuvem no LWW); registrado em `FALHAS_CONHECIDAS`
