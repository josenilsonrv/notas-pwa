...
- **[conftest.py 1-84 ~]** `# 🧪 [INÍCIO: TESTE - BACKEND/CONFTEST]` -> `DRIVER=memory` + `SUPABASE_URL`/`SUPABASE_ANON_KEY`/`SUPABASE_SERVICE_ROLE_KEY`/`SUPABASE_DB_URL` = `""` **antes** de importar o backend (hermeticidade preservada mesmo com `.env` real); `RAIZ`/`AQUI` no `sys.path`; fixtures `config_memoria`, `repo` e **`repo_qualquer`** (roda o MESMO contrato nos dois drivers)
...
- **[.env.example 1-52 ~]** `(sem comentário de ancoragem)` -> `SUPABASE_URL`/`ANON_KEY`/`SERVICE_ROLE_KEY`/`BUCKET_ANEXOS`/`DB_URL` (**com a nota do IPv6-only/pooler e do percent-encode da senha**), `SESSAO_*`, `ORIGENS_PERMITIDAS`, `HOST`/`PORTA`, `DRIVER`, `RATE_*`, `ANEXO_LIMITE_MB`/`ANEXO_COTA_MB` (a `ANON_KEY` é lida mas **não usada**: só a `SERVICE_ROLE_KEY` sai do backend)
