// 🧪 [INÍCIO: TESTE - NOTES PASTE SELECTION]
/*
 * Portado automaticamente de produtividade-ferrramenta/tests/notes_paste_selection.cjs por tools/portar-testes.cjs.
...
(async()=>{
 ...
 await load('<p>Existente</p>');
 await page.evaluate(()=>{const e=document.getElementById('notesEditor'),r=document.createRange();r.setStart(e,0);r.collapse(true);getSelection().removeAllRanges();getSelection().addRange(r);});
 await paste({'text/plain':'Novo '});assert.deepEqual(await text(),['Novo Existente']);
 await page.evaluate(()=>{const r=document.createRange();r.selectNodeContents(document.getElementById('notesEditor'));getSelection().removeAllRanges();getSelection().addRange(r);});
 await paste({'text/plain':'Substituído'});assert.deepEqual(await text(),['Substituído']);
 await page.evaluate(()=>{getSelection().removeAllRanges();app.notesSelectionRange=null;});await paste({'text/plain':'Sem seleção '});assert.ok((await text()).join('').includes('Sem seleção'));
 assert.deepEqual(errors,[]);console.log('OK: paste on editor root, Select All, and missing selection');
 }finally{await browser.close();}
})().catch(e=>{console.error(e);process.exit(1);});
// 🧪 [FIM: TESTE - NOTES PASTE SELECTION]
