...
(async()=>{
 ...
 await load('<p>Texto original</p><p>Destino </p>');await select(0,0,0,5);
 await page.evaluate(()=>{window.clip=new DataTransfer();document.getElementById('notesEditor').dispatchEvent(new ClipboardEvent('cut',{bubbles:true,cancelable:true,clipboardData:clip}));});assert.equal((await text())[0],' original');
 await select(1,8);await page.evaluate(()=>document.getElementById('notesEditor').dispatchEvent(new ClipboardEvent('paste',{bubbles:true,cancelable:true,clipboardData:clip})));
 assert.deepEqual(await text(),[' original','Destino Texto']);assert.equal(await rows().nth(1).locator('[style*="background"]').count(),0);
 await load('<p><strong><span style="background-color:#ffd700">abcdef</span></strong></p><p>Destino </p>');await select(0,2,0,4);await page.evaluate(()=>{window.clip=new DataTransfer();document.getElementById('notesEditor').dispatchEvent(new ClipboardEvent('cut',{bubbles:true,cancelable:true,clipboardData:clip}));});
 await select(1,8);await page.evaluate(()=>document.getElementById('notesEditor').dispatchEvent(new ClipboardEvent('paste',{bubbles:true,cancelable:true,clipboardData:clip})));
 assert.equal(await rows().nth(1).locator('strong span').textContent(),'cd');assert.equal(await rows().nth(1).locator('strong span').evaluate(e=>e.style.backgroundColor),'rgb(255, 215, 0)');
