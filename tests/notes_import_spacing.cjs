// 🧪 [INÍCIO: TESTE - NOTES IMPORT SPACING]
/*
 * Portado automaticamente de produtividade-ferrramenta/tests/notes_import_spacing.cjs por tools/portar-testes.cjs.
...
(async()=>{
 ...
 await load('<p></p>');await select(0,0);await paste({'text/html':'<p><strong>Objetivo:</strong> Aprender<br>\n<strong>Timeline:</strong> 7 meses<br>\n<strong>Stack Final:</strong> TypeScript</p>'});
 assert.equal(await rows().count(),3);assert.deepEqual(await text(),['Objetivo: Aprender',' Timeline: 7 meses',' Stack Final: TypeScript']);
 const gaps=await rows().evaluateAll(rows=>rows.slice(1).map((row,i)=>row.getBoundingClientRect().top-rows[i].getBoundingClientRect().bottom));assert.ok(gaps.every(gap=>gap<=7));
 await load('<div class="notes-line"><div class="notes-line-text">Objetivo</div></div><div class="notes-line"><div class="notes-line-text"><strong></strong></div></div><div class="notes-line"><div class="notes-line-text">Timeline</div></div>');assert.equal(await rows().count(),2);
 assert.deepEqual(errors,[]);console.log('OK: BR + HTML newline produces one line break; legacy empty fragments removed');
 }finally{await browser.close();}
})().catch(e=>{console.error(e);process.exit(1);});
// 🧪 [FIM: TESTE - NOTES IMPORT SPACING]
