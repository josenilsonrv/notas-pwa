...
(async()=>{
 ...
 await load('<h1>Acima</h1><p>Primeira linha</p><p>Segunda linha</p><h1>Abaixo</h1>');
 const result=await page.evaluate(async()=>{const rows=[...document.querySelectorAll('#notesEditor .notes-line')];rows[0].dataset.collapsed='false';app.refreshNotesCollapseControls();const y=()=>rows[3].getBoundingClientRect().top,start=y();const moving=app.animateNotesLineCollapse(rows[0]);await new Promise(r=>setTimeout(r,160));const middle=y();await new Promise(r=>setTimeout(r,140));const nearEnd=y();await moving;return {start,middle,nearEnd,end:y()};});
 console.log(result);assert.ok(result.middle<result.start);assert.ok(result.middle>result.end);assert.ok(Math.abs(result.nearEnd-result.end)<3);assert.deepEqual(errors,[]);console.log('OK: following title moves continuously without final spacing jump');
