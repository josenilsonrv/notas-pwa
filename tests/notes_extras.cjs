...
(async()=>{
 ...
 await page.evaluate(()=>{
  ...
  window.loadNote=async html=>{clearTimeout(app.notesSaveTimer);app.notesSession=null;app.projectsData[0].notas=html;/* PWA: os MODELOS sao locais (notas-pwa-templates) e o reset do storage nao pode apaga-los */const __modelos=localStorage.getItem('notas-pwa-templates');localStorage.clear();if(__modelos)localStorage.setItem('notas-pwa-templates',__modelos);await app.openNotesModal(1);};
